...
1. Resolve the configured installation targets and verify only the update package identity, manifest, immutable package receipt, packaged capability contract, skill, deterministic state helper, hook registration, and SessionStart payload needed to make the update safe. Every managed-file hash comes from that receipt. A supported manual or legacy source also requires `--expected-package-version <version>` and `--expected-receipt-sha256 <sha256>` copied from separately published release metadata. Validate both external values, the package state `release`, and the exact receipt bytes before reading or writing any installation target. Reject missing, malformed, wrong-release, wrong-version, wrong-pin, mismatched, development, or dirty developer sources. Repository tests, release audits, Mission Control checks, and codebase review remain developer or release work outside Doctor.
2. Classify the target owner before repair. For a marketplace-managed installation, report `reinstall_required` and use the supported plugin update or reinstall operation; never rewrite its cache. The plugin manager supplies that package identity, so Doctor does not require the manual external-pin arguments for a known marketplace path. For a supported legacy or manual installation, run `scripts/codex_coordinator_doctor.py --apply --expected-package-version <version> --expected-receipt-sha256 <sha256>` once. Every required skill and hook destination directory must already exist as a verified directory; missing, linked, redirected, replaced, or unprovable directories return `manual_action_required` before managed-target access or repair. With safe existing directories, Doctor replaces only receipt-declared managed files, validates each atomic replacement, preserves unexpected files and project state, and restores the captured last-known-good bytes if any write or installed-runtime check fails.
3. Verify the installed implementation itself: exact managed-file hashes, capability-contract version and required behavior markers, active-by-default enabled-repository lifecycle, user-only exclusions, report-only pause, durable-complex-only worker creation, parent-owned microtask execution, the one-to-three normal worker target and five-worker ceiling, direct first-turn assignment, control-first Coordinator role, direct-request archived-owner recovery without a repeat-confirmation phrase, the end-of-turn continuation gate and repository-heartbeat monitoring, inherited model plus cost-safe `low`/`medium` reasoning defaults, native task-lifecycle guidance, deterministic state-helper syntax, Coordinator skill frontmatter, every internal skill link, hook syntax, and a bounded isolated hook smoke run. Then repeat the helper with `--check` to prove the installed implementation is current and healthy.
...
5. Treat a missing, stale, incomplete, or self-contradictory capability contract as a failed installed implementation even when hashes, frontmatter, links, and hook syntax pass. Doctor reports installation integrity and recovery separately so callers can distinguish `healthy`, `local_modification_detected`, `trusted_repair_available`, `reinstall_required`, `repair_failed_last_good_restored`, and `manual_action_required`. If package identity, copying, installed-runtime validation, or the final check fails, keep or restore the last working installation where possible and report the exact receipt and defect. Do not fall back to partial hand edits.
...
Mission Control records installed-implementation integrity and the enabled-project scan as separate results. A manual package without valid external release pins remains `manual_action_required` without target access or repair; a marketplace-managed package without plugin-manager trust remains `reinstall_required` without package/cache execution or writes. Either installation result is retained as an action instead of being called healthy, while Mission Control still calls `apps.mission_control.doctor_scan.DeterministicDoctorScanner` with the enabled project roots it already knows. The scanner returns structured issue codes, counts, coverage limits, read counters, and zeroes for application files, transcript bodies, model calls, and model tokens. No model is invoked, so a model never receives project paths, task URLs, transcript text, or application files. A verified mismatch may produce one deduplicated append-only finding; an unavailable input becomes an explicit limitation rather than a healthy claim.
...
A scheduled Doctor for a manual installation runs the installation helper with the separately configured expected version and receipt SHA-256 on both `--compact --apply` and `--compact --check`, then runs `python -m apps.mission_control.doctor_scan --compact --write-findings` with only the previously disclosed `--project-root` targets. A marketplace schedule uses the supported plugin-manager update or reinstall path instead of manual repair. The scanner may query native SQLite task metadata and bounded rollout event prefixes for those exact Coordinator or active-worker identities; it never parses message content. Newly discovered projects are not added to a write-enabled schedule without the fresh notice and approval required by maintenance policy.
