...
1. Resolve the configured installation targets and verify only the update package identity, manifest, packaged capability contract, skill, deterministic state helper, hook registration, and SessionStart payload needed to make the update safe. Repository tests, release audits, and codebase review remain developer or release work outside Doctor.
...
- more than five non-terminal project-execution workers, excluding the Coordinator and Maintainer, unless canonical task history records a direct user run-wide override;
...
Time, `idle`, or `notLoaded` alone never proves that a task is abandoned, forgotten, stale, or unusable. A report that arrived while the Coordinator was running is not unattended unless a later Coordinator reconciliation boundary passed without disposition. Do not wake a task to ask what it is doing; use native status, recent turns, and project records.
