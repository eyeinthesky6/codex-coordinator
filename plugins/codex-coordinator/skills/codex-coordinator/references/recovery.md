...
When a completed worker or direct user request says another owner blocks required scope, inspect that exact owner's native status in the same turn before declaring a blocker or writing a waiting handoff. Canonical ownership proves who last held the boundary, not that the owner is still live. If the owner is running, preserve ownership and follow the blocked-owner handoff. If it is confirmed archived, terminal, or unusable, reconcile the stale boundary and transfer the unfinished work through the recovery rules below. The user's original request to execute or continue the goal already authorises that recovery; never ask the user to ping the old task, repeat an exact phrase, or approve the replacement again. The user never relays the internal prompt.
...
Do not ask the user to reopen threads, ping an archived task, repair state, repeat a prescribed sentence, or say “proceed” after already giving the action. The direct request that first exposes the archived owner is the next explicit coordinated action: apply the main skill's Coordinator creation authority for an archived Coordinator, or let the registered Coordinator replace an archived worker, in the same project and local checkout. Inherit configured model defaults unless the user explicitly authorised an override. Use a complete creation prompt containing verified project, epoch, role, task, ownership, pending transitions, acceptance criteria, and stop condition. Do not create a worktree unless the user or settings require it.
