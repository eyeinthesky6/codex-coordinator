...
6. The Coordinator gives the user one consolidated completed/active/queued/blocked summary at the end of each coordinating turn and when asked, rather than making the user inspect worker windows. The summary always names `Managing`, `Paused - report-only`, or `Attention needed` and lists excluded tasks or `none`, then says what finished, what remains, who owns it when useful, and which exact decision needs the user.
