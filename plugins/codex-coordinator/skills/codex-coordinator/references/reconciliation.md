...
### Goal coverage and team-leader loop

The Coordinator leads delivery of the user's outcome. Worker status is input to that job, not the definition of success.

1. At the start of a shared goal, create a goal-coverage ledger in the Coordinator task history from the user's instructions, explicit acceptance criteria, required prerequisites, and retained constraints. Add later direct decisions and newly proven necessary work. Do not invent unrelated improvements or treat an optional idea as required without recording that choice.
2. Give every goal item one plain disposition: `COMPLETED`, `ACTIVE`, `QUEUED`, `AWAITING_USER_DECISION`, `ENVIRONMENT_BLOCKED`, `REJECTED`, or `NOT_NEEDED`. Record its evidence or exact remaining result, current owner when one exists, dependency, and next action. An unassigned item remains visible in the ledger; absence of a worker is not a disposition.
3. Reconcile worker completion against the goal item it was meant to advance. A finished worker may complete the item, expose follow-up work, fail acceptance, or prove a blocker. Closing every worker does not close the shared goal. Never infer goal completion from an empty worker list, native `idle`, a quiet heartbeat, elapsed time, or `no change` since the last check.
4. After every material result or status check, compare the ledger with the user's requested outcome and select the highest-value executable action on the critical path. Continue or assign authorised work when it fits the shared goal. Where a worker lacked a tool or environment, check for another currently available in-scope tool, host, or coherent owner that can complete the same acceptance criterion before calling it blocked.
5. If no capable execution path exists, record the exact missing capability, what was tried, the evidence still needed, and the smallest practical recovery path. For example, reassign browser UAT to a browser-capable task rather than leaving it attached to a finished task that lacked browser access.
6. When user authority or a product choice is genuinely required, keep the goal non-terminal and ask one concise, prioritised decision request. State the exact action, target, benefit, downside, recommended choice, and what will happen after approval. Batch related decisions when that reduces interruption, but do not hide the critical-path decision among optional choices.
7. For optional work, recommend `do now`, `defer`, or `not needed` with a short reason and record the user's decision when provided. Optional does not mean forgotten, and it must not block the required goal unless the user makes it part of acceptance.
8. In each consolidated user update, lead with the shared-goal verdict, then say what completed, what remains, the next action the Coordinator can take, and the exact decision or environment gap that prevents it. Suggest a correction or safer next path when evidence shows the current plan cannot deliver the goal. Never reduce the report to worker activity.

The leader loop ends only when the full goal-coverage ledger is terminal with evidence, the user deliberately stops or supersedes the goal, or the remaining item is honestly recorded as blocked with no authorised recovery path. `No worker is running` and `no new result` are non-terminal observations whenever ledger items remain.
...
At goal start, after material Git changes, and before closure, inspect the relevant GitHub state when read access is available: pull requests, immutable heads, base branches, required checks, reviews, mergeability, unresolved conversations, merge state, and issues that may affect the shared goal. Put each material provider result in the same goal-coverage ledger as local work. Keep unrelated issues outside the objective unless the user directly adds them. Read-only monitoring needs no repeated consent; if access or a required field is unavailable, record the exact gap and do not infer a healthy or merge-ready state.

Provider reads and provider mutations have different authority. A commit, push, pull-request create or update, mark-ready action, review submission, issue mutation, merge, release, deployment, repository-setting change, or other provider write requires exact current user consent unless the same bounded action, target, and action class are already directly authorised and recorded. General filesystem access, a prior read request, or a broad instruction to monitor is not mutation consent.

Before asking for merge consent, report the exact pull request, immutable head, base, required checks, reviews, mergeability, unresolved conversations, intended merge method, stacked-work effect, and recommendation. Immediately before any authorised provider mutation, read the target again and verify that its immutable identity and every consent condition still match. Stop on drift, ask again when the changed state affects the decision, and after mutation return the exact provider receipt rather than treating an attempted call as success.

### Project-related scheduled-task reconciliation

At coordinated-goal start, after a material task or automation change, and before closure, inspect all available project-related native automations, Coordinator heartbeats, materially changed run receipts, and repository-native scheduled workflows. Bind scheduled work by exact project identity, working directory, target, and action boundary, not display name alone. For every matched item, reconcile its purpose, status, cadence, current authority, file or action ownership, latest material result, dependencies, relationship to the shared objective, and next action into the canonical goal ledger.

Keep unrelated scheduled work outside the current objective. Preserve user-created, paused, disabled, and explicitly stopped automations unless a direct user decision authorises a change. Read-only inspection needs no repeated permission. A minor alignment may proceed only when it is bounded, reversible, already inside direct user authority, and does not expand the automation's action class or ownership.

Record a direct user decision before any major scheduled-task change. Major means enabling, disabling, or deleting an automation; meaningfully changing cadence, purpose, scope, repository, account, identity, or target; adding an external publish, message, moderation, deletion, security, release, deployment, or provider-write action; changing credentials, cost, permissions, destructive behavior, or file-write ownership; or crossing a project boundary. Bring each material scheduled-run result into the next user-visible delivery summary. An HTTP success, green run, or completed status proves only that bounded run, not the wider product outcome without its required outcome evidence.
...
6. Before every user-visible Coordinator final response, reconcile changed managed turns, pending inbox records, canonical ownership, acceptance evidence, provider state, scheduled-task state, retained decisions, and the complete goal ledger. Preserve every unchanged non-terminal item and verify one real continuation or return path. The response always names `Managing`, `Paused - report-only`, or `Attention needed`, lists excluded tasks or `none`, and includes plain-language sections for done work, pending work, blockers or decisions, next actions, and the full-goal verdict. Write `None` for an empty section. A quiet no-change heartbeat remains silent, preserves pending state, and never converts it to done.
...
Repository enablement authorises exactly one native heartbeat attached to the pinned Coordinator. It remains while the repository is enabled, including workload-idle and report-only periods, and is removed only when the user disables Coordinator for that repository.
...
- **Goal terminal, repository still enabled:** no active, queued, blocked, paused, pending-command, resume-queue, unresolved-decision, unprocessed-inbox, or unreconciled worker-turn item remains. Close the goal honestly, set the Coordinator workload status to `IDLE`, and retain its registration, pin, acceptance, mode, exclusions, and repository heartbeat.
...
Do not treat a paused item or user decision as terminal. A quiet heartbeat may keep that durable state reconciled without repeated user-facing reminders. Do not mark the workload idle while actionable records or tasks remain. The final consolidated summary must state the mode and exclusions, then say what is complete, active, queued, blocked, or awaiting a decision; it must not leave open work visible only in worker windows.
...
2. On each heartbeat, read only same-repository tasks whose native turn changed plus unprocessed inbox records. List excluded tasks and native status without managing them or reading their private content. When the host exposes a native incremental cursor such as `afterCursor`, pass the last cursor returned in the current Coordinator flow and let Codex own its task history and invalidation. Do not persist or mirror native turns in project cache. If the cursor is missing, invalid, or lost after recovery, do one bounded native read and continue with the new native cursor. Reconcile completed work, blockers, ownership, capacity, and pending decisions; in `MANAGING`, dispatch or wake only work already authorised by the shared goal; in `REPORT_ONLY`, perform no control action.
3. Produce no cross-task message and no user-facing status when nothing material changed. If work completed, became blocked, or needs a decision, update canonical state and give one consolidated summary in the Coordinator task.
   A no-change heartbeat preserves the existing goal-coverage ledger exactly. It is not a completion event and must not turn `ACTIVE`, `QUEUED`, `AWAITING_USER_DECISION`, or `ENVIRONMENT_BLOCKED` work into an idle or terminal goal.
4. Keep the Coordinator-owned repository heartbeat after shared-goal completion and workload idle. Delete it only when the user disables Coordinator for that repository; never delete an automation the user created independently.
