...
6. Every task in an enabled repository is managed by default, including small, isolated, read-only, maintenance, and status work. Managed means visible to the Coordinator with a recorded boundary; it does not mean waiting for Coordinator approval. Default management does not require creating a worker or heavy contract for a microtask: the pinned Coordinator may observe and record it as direct work when ownership is conflict-free. Only a direct user exclusion removes an exact task from management.
7. If a registered Coordinator exists and is usable, verify its exact address, epoch, acceptance, current mode, and exclusions; do not self-elect. A fresh same-repository task that is not excluded writes one project-local `DIRECT_USER_NOTICE` with `state: PROCEEDING` when conflict-free work can continue, then starts immediately without acknowledgement. It writes one `COORDINATION_REQUEST` with `state: WAITING` only when a recorded dependency, pause, pending control command, unresolved user decision, or ownership conflict prevents safe work. Native `idle`, `notLoaded`, or a submitted message that has not started running is not a coordination wait. In `REPORT_ONLY`, it may continue only within direct user authority and conflict-free ownership while the Coordinator observes without control actions. It does not send a native registration request or wait for a registration acknowledgement.
8. When the main skill's Coordinator creation authority applies, use native `list_projects` and require exactly one local project whose normalized `path` equals the resolved primary-worktree path; never select by label, name, recency, or similarity. If no unique exact match exists, report that blocker and create nothing. Otherwise pass its `projectId` to `create_thread`, target the local primary checkout, and bind the returned exact thread as the sole bootstrap grantee. The repository Coordinator itself never starts in a linked worktree; the later worker-placement policy may select bounded worktrees for independent writers. Omit the model unless an allowed override applies and pass native `thinking: "medium"` by default; use `thinking: "low"` only for a deterministic, reversible coordination goal.
...
9. Record the delegation decision before ordinary implementation starts: `REUSE` for the same coherent area and core goal; `RETAIN` only for a routine microtask or tightly coupled control/integration step; `DELEGATE` for substantial independent work; or `CREATE` only when the area is substantial, unrelated, and has no reusable owner. Include the short evidence in the task contract or goal ledger so control-first behavior is reviewable without creating a second policy store.
...
6. Rename a generated generic title once with `codex_app__set_thread_title` when the native surface supports it. A direct request to coordinate the goal supplies consent for this bounded cleanup. Use a short stable coherent work-area or current-goal name, not a protocol ID; preserve a meaningful user-written title and do not rename repeatedly for small steps. A rename failure does not change identity or authority.
...
Before waking, resuming, replacing, or creating a worker, inspect the relevant registered tasks in canonical state and their exact native status. Reconcile each task against the current shared goal, its acceptance criteria, pending transitions, blockers, paused or resume entries, and task-specific ownership; do not infer unfinished work merely because a task exists in history.
...
6. Continue unfinished work under its existing user authority only when it remains necessary for the same current goal and can resume safely inside its recorded scope after dependency, ownership, and native-status checks. Ask the user before reviving optional work, work from a superseded objective, materially expanded behavior or architecture, a conflicting prior instruction, costly work, destructive or external action, or any item whose relevance or authority is unclear. Record unclear items as `AWAITING_USER_DECISION` and ask one concise prioritised question.
7. In the next consolidated user update, report counts of material historical items closed, continued under existing authority, deferred or not needed, and awaiting a decision. Do not make the user inspect old task windows.
...
Subagents remain available inside a Coordinator or worker turn when useful. They are parent-owned helpers rather than separate durable project sessions: the registered parent keeps the task contract and ownership, validates their output, and includes their work in its own final result and reconciliation. Use one to three parent-owned subagents when at least two independent, bounded lanes can materially shorten the turn, such as separate inspections, deterministic test lanes, narrow read-only audits, or disjoint low-risk one-or-two-file fixes inside the parent's recorded boundary. Do not spawn them for a single trivial command, tightly sequential work, a lane whose coordination cost exceeds its value, or shared-file work that must be serialized. Give each helper an exact non-overlapping boundary and stop unused lanes once enough evidence exists. Do not create a project task, canonical session, task contract, inbox registration, or user-visible Codex task for the subagent. Do not place an independent Codex task UUID in an agent-tree messenger, and do not claim that a subagent owns canonical scope separately from its parent.
...
- execution placement chosen by the Coordinator: shared checkout or a bounded linked worktree, with the reason recorded;
...
5. The Coordinator may place an independent writer in a bounded linked worktree without a separate user request when all of these are true: its goal and write scope are exact and disjoint; using the shared checkout would make it wait behind another writer or dirty integration state; parallel execution shortens the critical path; and one named integration owner will review and integrate the result. Respect any user or project setting that requires or forbids worktrees.
6. Keep the primary worktree as the only canonical Coordinator-state location. A linked worker reads project identity and current epoch from the primary worktree, owns only its recorded product paths and branch, and never edits `CURRENT.md`, task contracts, or another task's inbox record.
7. Record the worker checkout, branch, integration owner, and return evidence in its task contract. The native Codex and Git surfaces create and maintain the linked worktree; Coordinator selects the placement and tracks the handoff but does not invent a second worktree lifecycle.
