...
- Documents are authoritative. Messages are a sparse control channel, not an activity feed: routine progress, findings, reviews, and completion stay in the worker's own turn for the Coordinator to pull and reconcile.
...
- Only the active Coordinator edits `CURRENT.md` and project-execution task files. Every registered worker writes one append-only `TURN_RECONCILIATION` record at the end of each material turn; other inbox records follow the narrower operations rules. These records are reports, never ownership or permission. Use the bundled [state helper](scripts/coordination_state.py) for deterministic shape checks, safe task/inbox file creation, harmless taskless-value normalization, and reconciliation-ledger validation. The Coordinator validates and merges reports into canonical state.
...
- Keep one durable worker thread per coherent work area and reuse it for later steps of that same goal. Do not create separate threads merely for that area's audit, implementation, tests, documentation, or follow-up findings when one owner can handle them coherently. Default to at most five non-terminal worker threads per project run, excluding the Coordinator; use fewer when extra parallelism adds little, and exceed five only under a direct user override.
...
- Independent Codex tasks and collaboration subagents have different address spaces. Subagents remain available as parent-owned helpers: the registered parent task retains scope, ownership, evidence, and reporting responsibility. Deliver messages to user-visible Codex tasks only with the app-native thread messenger using the exact verified thread UUID and host when exposed; never pass a Codex thread ID to an agent-tree messenger. On native resolution failure, retry exact native discovery once, then use the project-local inbox or completed-turn path.
