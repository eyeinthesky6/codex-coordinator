...
- While a coordinated goal has live work, keep one temporary native heartbeat on the Coordinator when the host exposes heartbeats and the user's request is to coordinate or keep the goal moving. It reconciles only changed native turns and durable records, stays quiet on no change, and is removed when the goal becomes terminal. If heartbeats are unavailable, permit at most one batched boundary wake for a ready result or blocker when the exact Coordinator is idle. Never send status pings, availability checks, acknowledgement chatter, or one message per finding.
- Independent Codex tasks and collaboration subagents have different address spaces. Subagents remain available as parent-owned helpers for short, standard operations inside the parent's existing boundary: the registered parent task retains scope, ownership, evidence, validation, and reporting responsibility, and the subagent creates no project task, canonical ownership, or user-visible durable thread. A worker may use them for narrow inspection, one deterministic command lane, or a low-risk one-or-two-file fix, then validates and reconciles the result itself. Deliver messages to user-visible Codex tasks only with the app-native thread messenger using the exact verified thread UUID and host when exposed; never pass a Codex thread ID to an agent-tree messenger. On native resolution failure, retry exact native discovery once, then use the project-local inbox or completed-turn path.
...
7. At the start and end of its turn, and on its one temporary heartbeat while live work remains, the Coordinator reconciles changed records against native task status, actual pending work, ownership, and acceptance evidence; it closes completed items, continues or assigns authorised work, and records a real user decision when approval is required.
8. Update canonical state and give the user one consolidated completed/active/queued/blocked summary. Do not declare the goal complete or idle while a report, promise, blocker, pending command, paused item, or required continuation lacks a recorded disposition.
