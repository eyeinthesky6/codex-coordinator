...
- Apply the model-and-reasoning precedence in the operations lane for every new task: managed constraints first, then an explicit per-task or run-wide user override, then host or project defaults and task-fit guidance. Never silently replace an exact user choice with a different model or unsupported reasoning level.
