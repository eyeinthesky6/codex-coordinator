...
from .lifecycle import lifecycle_lock, read_state, write_state


APP_DIR = Path(__file__).resolve().parent
STATIC_DIR = APP_DIR / "static"
PLUGIN_ROOT = APP_DIR.parent
LOGO_PATH = PLUGIN_ROOT / "assets" / "logo.png"
...
class MissionControlRuntime:
    def __init__(self, roots: list[Path], codex_home: Path | None, data_dir: Path):
        ...
        self.doctor = DoctorRunner(data_dir, PLUGIN_ROOT, self.collector.codex_home)
        self.deep_review = DeepReviewRunner(
            data_dir, PLUGIN_ROOT, self.collector.codex_home
        )
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/shutdown":
                request = self._read_json()
                if request.get("confirmation") != "user-requested-shutdown":
                    raise ValueError("Shutdown requires explicit user confirmation.")
                self.server.request_shutdown()  # type: ignore[attr-defined]
                self._send_json({"status": "stopping"}, HTTPStatus.ACCEPTED)
                return
        except ValueError as error:
...
class MissionControlServer(ThreadingHTTPServer):
    ...
    def request_shutdown(self) -> None:
        lifecycle_path = self.runtime.settings.data_dir / "lifecycle.json"
        with lifecycle_lock(lifecycle_path):
            value = read_state(lifecycle_path)
            write_state(
                lifecycle_path,
                enabled=False,
