...
- Advanced the installed capability contract to version 21 for provider monitoring and mutation consent, complete every-turn delivery summaries, project-related scheduled-task reconciliation, and externally pinned installed-core integrity.
- Added an immutable managed-file package receipt. Manual repair now requires the expected package version and receipt SHA-256 from separately published release metadata before any installation target is read. Doctor rejects dirty, mismatched, development, wrong-version, and wrong-pin sources; reports marketplace drift as reinstall-required without rewriting cache files; and limits supported legacy/manual repair to declared files with exact rollback receipts and last-known-good restoration.
- Kept this behavior in the global plugin package rather than project state. The manifest retains `0.3.0` as the public baseline, while development package state and receipt version `0.0.0-unreleased` prevent these source bytes from masquerading as that release; choosing a real version, tag, release, and external receipt pin remains separately authorised. Existing installations then use the documented marketplace replacement and plugin reinstall flow.
