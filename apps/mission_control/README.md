...
Manual Doctor runs request GPT-5.6 Sol with Extra High reasoning because they can assess and repair cross-project coordination infrastructure. Mission Control never downgrades Doctor to an older model family. If the `codex` executable on `PATH` is too old for Sol, the run fails with an upgrade message instead of silently spending tokens on GPT-5.5.
